...
#else
static int nilfs_readdir(struct file *file, void *dirent, filldir_t filldir)
#endif
{
#if HAVE_NEW_READDIR
	loff_t pos = ctx->pos;
...
#else
			file->f_pos += PAGE_CACHE_SIZE - offset;
#endif
			return -EIO;
		}
...
#if HAVE_NEW_READDIR
				over = !dir_emit(ctx, de->name, de->name_len,
...
#else
			file->f_pos += nilfs_rec_len_from_disk(de->rec_len);
#endif
		}
		nilfs_put_page(page);
	}
